...
class BinanceOrder(OrderBase):
    def __init__(self, owner, data, exectype, binance_order):
        ...
        if self.exectype == Order.Market:
            ...
            # self.price = sum(float(fill['price']) for fill in binance_order['fills']) / len(
            #     binance_order['fills'])  # Average price
...
class BinanceBroker(BrokerBase):
    ...
    def _submit(self, owner, data, side, exectype, size, price, hedge=False):
        type = self._ORDER_TYPES.get(exectype, ORDER_TYPE_MARKET)
        symbol = data._name
        binance_order = self._store.create_order(symbol, side, type, size, price, hedge)
        ...
        if binance_order['status'] in [ORDER_STATUS_FILLED, ORDER_STATUS_PARTIALLY_FILLED]:
            avg_price = float(binance_order['avgPrice'])
            comm = float(binance_order['cumQuote']) * 0.0005
    ...
    def sell(self, owner, data, size, price=None, plimit=None,
        ...
        try:
            ...
            print('Exception while selling:', e)
            return None
